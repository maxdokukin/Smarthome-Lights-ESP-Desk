...
void setup() {
  ...
  deskLights = new LedController(&deskStrip, DESK_LED_NUM, 4000);
  ceilingLights = new LedController(&ceilingStrip, CEILING_LED_NUM, 4005);
...
void eepromInit(){
  ...
  // //read from memory
  // byte *params = memory->getDeskParams();

  // //set params 
  // deskLights->setRGB(params);
  // deskLights->setBrightness(map(params[3], 0, 255, 0, 100.0));
  ...
  // deskLights->setMode(static_cast<lightsMode>(params[4]));

  //sync homekit values
//   cha1_hue.value.float_value = deskLights->getHue();//hue
//   cha1_sat.value.float_value = deskLights->getSat();//sat
//   cha1_bright.value.int_value = deskLights->getBrightness();//bright
//   cha1_on.value.bool_value = deskLights->getState();//on/off
//   cha1_switch_on.value.bool_value = deskLights->getMode();//mode
}
