...
class PerlinFade{
  ...
  PerlinFade(uint16_t hs, uint16_t hg, byte fs, byte mib, byte mab, byte mis, byte mas){
  ...
  void frame(){
    ...
    for(int i = 0; i < NUM_LEDS; i++)
      pixels.setPixelColor(i, getFireColor((inoise8(i * fireStep, counter))));
